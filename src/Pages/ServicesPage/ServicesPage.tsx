...
import { getServicesAPI, GetServices, deleteServiceAPI, createServiceAPI, updateServiceAPI } from '../../Services/Management/ServicesService';
...
const ServicesPage = () => {
  ...
  const [searchParams, setSearchParams] = useState<GetServices>({
    pageNumber: 1,
    pageSize: 10,
  });
  ...
  const fetchServices = async () => {
    ...
    try {
      const response = await getServicesAPI(searchParams);
      setServices(response.data);
    } catch (err) {
      setError('Ошибка загрузки услуг');
      toast.error('Не удалось загрузить данные услуг');
    } finally {
      setLoading(false);
    }
  };
  ...
  const formatDate = (date: Date) => {
    ...
    const { register: registerCreateForm, handleSubmit: handleCreateSubmit, reset: resetCreateForm, formState: { errors: createErrors } } = useForm<{name: string}>();
      ...
      const { register: registerUpdateForm, handleSubmit: handleUpdateSubmit, reset: resetUpdateForm, formState: { errors: updateErrors } } = useForm<{name: string}>();
  
      const handleUpdateService = async (data : {name: string}) => {
        if(typeof(currentService?.id) === "string") {
          try {
  ...
  }, [currentService, resetUpdateForm]);
        ...
        {isUpdate && (
                      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                        <div className="bg-white rounded-lg p-6 w-full max-w-2xl">
                            <h2 className="text-xl font-bold mb-4">Обновление услуги</h2>
          ...
          {isCreate && (
                      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                        <div className="bg-white rounded-lg p-6 w-full max-w-2xl">
                            <h2 className="text-xl font-bold mb-4">Создание новой услуги</h2>
                            <form onSubmit={handleCreateSubmit(handleCreateService)}>
                                <div className="mb-4">
                                    <label className="block text-sm font-medium text-gray-700 mb-1">
                                        Название услуги
                                    </label>
                  ...
                  {services.map((service) => (
                    <tr key={service.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                        <button
                          onClick={() => {setCurrentService(service); setIsUpdate(true)}}
            ...
            <div className="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6">
              <div className="flex-1 flex justify-between sm:justify-end">
                ...
                <button
                  ...
                  disabled={services.length < (searchParams.pageSize ?? 10)}
